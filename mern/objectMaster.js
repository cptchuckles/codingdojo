...
const fireTypes = pokémon.filter(p => "fire" in p.types);
...
const poisonOnly =
    pokémon
    .filter(p => p.types == "poison") // array to string coercion hack
    .map(p => p.name);

const firstTypeOfFlyingSecond =
    pokémon
    .filter(p => p.types[1] === "flying")
    .map(p => p.types[0]);

const normalCount = pokémon.reduce((a, p) => a + ("normal" in p.types ? 1 : 0), 0);
